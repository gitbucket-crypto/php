...
function nuceport()
{
    if(PHP_OS=='Linux')
    {
        ...
        global $usada, $total, $livre, $load, $iddle;
        
        $mem = (string)getMemory();
        $mem = (explode(' ', $mem));
        //var_dump($mem); exit;
        if(isset($mem['48']) && $mem['48']!='')
        {
            $total = $mem['48'];
        } 
        if(isset($mem['53']) && $mem['53']!='')
        {
            $total = $mem['53'];
        }        
        if(isset($mem['81'])  && $mem['81']!='')
        {
            $usada = $mem['81'];
        }
        if(isset($mem['61'])  && $mem['61']!='')
        {
            $usada = $mem['61'];
        }
        if(isset($mem['62'])  && $mem['62']!='')
        {
            $usada = $mem['62'];
        }
        ...
        $cpu = explode(' ',$cpu);
        //var_dump($cpu); exit;
        if($cpu=='sh: 1: iostat: not found')
	    {
			$cpuUsage=' error lib no exists';
		}
        ...
        $disk = explode(' ', $disk);
        //var_dump($disk); exit;
        if(isset($disk['22']) && $disk['22']!='')
        {
            $htd=$disk['22'];
        }
         if(isset($disk['27']) && $disk['27']!='') 
        {
            $htd=$disk['27'];
        }
        print($htd."\r\n");
