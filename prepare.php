...
function init()
{
    if(PHP_OS== "Linux")
    {
        ...
        if(trim($py)=='')
        {
            shell_exec(bash.' && sudo apt install python3.9-full python3.9-dev python3.9-venv python3-pip pip  -y');
            init();
        }
...
function installENV()
{
    if(PHP_OS == "Linux")
    {
        ...
        $pip= shell_exec(bash.'&& sudo dpkg -l pip');
        if(strpos($pip, 'dpkg-query: não foram encontrados pacotes coincidindo com')!=false)
        {
            shell_exec(bash.' && sudo apt-get install pip -y');
        }
		...
		{
			shell_exec(python.' -m pip install requests');
			shell_exec(python.' -m pip install requests --user');
		}
		if(find('selenium', $pip)==false)
		{
			shell_exec(python.' -m pip install selenium ');
			shell_exec(python.' -m pip install selenium --user');
		}
		if(find('speedtest-cli', $pip)==false)
		{
			shell_exec(python.' -m pip install speedtest-cli ');
			shell_exec(python.' -m pip install speedtest-cli --user');
		}
    }
    else
    {
        exec(python.' -m pip  --upgrade pip');
        exec(python.' -m pip  --upgrade virtualenv');
...
{
    if ($needle !== '' && stripos($haystack, $needle)) {
